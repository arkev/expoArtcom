...
$.TextboxList.Autocomplete.Methods = {
	...
	standard: {
		filter: function(values, search, insensitive, max){
			...
			var newvals = [], regexp = new RegExp('\\b' + escapeRegExp(foldAccents(search)), insensitive ? 'i' : '');
			for (var i = 0; i < values.length; i++){
				if (newvals.length === max) break;
				if (regexp.test(foldAccents(values[i][1]))) newvals.push(values[i]);
			}
			return newvals;
		},
		
		highlight: function(element, search, insensitive, klass){
			var regex = new RegExp('(<[^>]*>)|(\\b'+ escapeRegExp(foldAccents(search)) +')', insensitive ? 'ig' : 'g');
...
var accent_map = {'&aacute;':'a', '&eacute;':'é','&iacute;':'í','&oacute;':'ó','&uacute;':'ú','&ntilde;':'ñ','á':'a','é':'e','í':'i','ó':'o','ú':'u','ñ':'n'};
var foldAccents = function(str){
	if (!str) { return ''; }
	var ret = str;
	for (key in accent_map) {
		ret = ret.replace(new RegExp(key, 'g'), accent_map[key]);
	}
	return ret;
};
