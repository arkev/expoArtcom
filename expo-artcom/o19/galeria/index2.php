...
<head>
  ...
  <link rel="shortcut icon" href="../css/img/expo-artcom-p2012-favicon-2012-bw.png" type="image/png">
...
<div id="header" >
	<div id="menu-bar">
		<a href="../galeria">
			<img id="logo" src="images/logo.png" height="15px" style="padding:12px 0 0 0;float:left;" />
		</a>
		<div id="menu-container">
			...
			<a href="../../p12" class="link">registro</a>
		</div>
	</div>
</div>
<br>
...
<script>
	//arreglo de imagenes qe aparecen al principio
	var boxes = [];
	var primeras = [];
</script>
	<?php
          ...
          while($row = mysql_fetch_assoc($obras)){
            ...
            $nombresApellidos1 = "";
             ...
             $files = mysql_query("select * from `archivo` where obra_id = ".$row['id']);
          	 ...
          	 <a onclick="lightbox('<?php echo $row['id'];?>');" class="various1" href="#inline<?php echo $row['id'];?>">
    			<div id="box" class="box photo col2" onmouseover="over('<?php echo $row['id'].$i;?>');" onmouseout="out('<?php echo $row['id'].$i;?>');">
			...
			while($row3 = mysql_fetch_assoc($files2)){
				...
				if(array_search($extension,$images) !== FALSE){		 
			 	?>
			 		<img src="../slir/w120-h120-c1:1/expo-artcom/p12/upload/<?php echo $row3['nuevo'];?>" alt="<?php echo htmlentities($row['titulo'])." - ".$row['id'].$i?>" /> 
			 	<?php
			 		$encontroImagen = true;
			 		break;
			 	}	
			...
			if($encontroImagen == false){
				?>
				<img src="http://placehold.it/120x120&text=<?php echo strtoupper($extension);?>" alt="<?php echo htmlentities($row['titulo'])." - ".$row['id'].$i?>" /> 
				<?php
			}
        ...
        style="border:none; overflow:hidden; width:150px; height:21px;" allowTransparency="true"></iframe>
								...
								<div id="imgs-container<?php echo $row['id'];?>" style="width:100%;background:black;">
									...
									if($contador<=60){//Cargar los primeros 60 proyectos, los demas se haran con js al desplegar su vista previa respectiva
									...
									?>
										...
										<img class='imgs' src='../slir/w120-h120-c1:1/expo-artcom/p12/upload/<?php echo $row4['nuevo'];?>'/>
										
									<?php
										   }else{
										   ?>
										   
										   <a target="_blank"href="../upload/<?php echo $row4['nuevo'];?>"><img class="imgs" src="http://placehold.it/120x120&text=<?php echo strtoupper($extension2);?>"></a>
										   <?php
										   }
										}
										$contador++;
...
<script>
  ...
  $(function(){
	...
	$('#append').click(function(){
		...
		document.getElementById('append').innerHTML="<img src='http://content.boostmobile.com/boostwebapp/images/loading_bar.gif' height=13/> &nbsp;cargando...";
  ...
  function getImagenes(){
		...
		while(count<=60){
			...
			if(proyectoAnterior!=str[10]){
				...
				var showImg = "http://placehold.it/120x120&text="+str[0].split(".")[1].toUpperCase();
				
				
				if(imgEncontrada==true){
					var str3 = boxes[jEncontrada].split("%");
					showImg = "../slir/w120-h120-c1:1/expo-artcom/p12/upload/"+str3[0];
				}
				...
				for(var n=0; n<boxes.length; n++){
					...
					if(arreglo[10]==str[10]){
						...
						if(extension.indexOf("png") != -1 || extension.indexOf("jpg") != -1 || extension.indexOf("jpeg") != -1 || extension.indexOf("gif") != -1){
							...
							document.getElementById("imgs-container"+str[10]).innerHTML = document.getElementById("imgs-container"+str[10]).innerHTML+" <img class='imgs' src='../slir/w120-h120-c1:1/expo-artcom/p12/upload/"+arreglo[0]+"'/>";
						
					
						}else{
						
							document.getElementById("imgs-container"+str[10]).innerHTML = document.getElementById("imgs-container"+str[10]).innerHTML + "<a target='_blank'href='../upload/"+arreglo[0]+"'><img class='imgs' src='http://placehold.it/120x120&text="+arreglo[0].split(".")[1].toUpperCase()+"'></a>";
		...
		<style type="text/css">
			...
			h3{ margin: 10px 10px 0 10px; color:#FFF; font:13pt Arial, sans-serif; letter-spacing:-1px; font-weight: normal;  }
		...
		<script type="text/javascript">
			...
			function lightbox(id){
				...
				window.history.replaceState("object or string", "Title", "projects.php?id="+id);
				
				document.getElementById("fb-like"+id).src = "http://www.facebook.com/plugins/like.php?href=http://artcom.um.edu.mx/expo-artcom/g/p12-"+id+"&send=false&layout=button_count&width=150&show_faces=false&action=like&colorscheme=light&font=lucida+grande&height=21";
    ...
    if(windowWidth<850){
           document.getElementById("logo").src = "images/logomini.png";
