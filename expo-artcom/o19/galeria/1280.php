...
<head>
  <meta charset="utf-8">
  <title>Fichas técnicas · Expo ARTCOM Portafolio estudiantil · primavera 2012</title>
  ...
  <style type="text/css" media="screen">
    ...
    .ficha{width: 1596px !important;}
    .ficha + .ficha{clear: both;border-top: 1px solid rgba(210,210,210,.6);margin-bottom: 1.75em;}
    .wrap{width: 1600px;margin: 0 auto;clear: both;}
    body{background: url('../css/img/skin_side_up.png') repeat fixed;}
  </style>
</head>
<body>
  <div class="wrap">
...
if(array_search($extension,$images) !== FALSE){
        echo "
      <img src=\"/i/w1280/expo-artcom/".$ccn."/upload/".$row3['nuevo']."\" alt=\"".htmlentities($row['titulo'])." - ".$row['id'].$i."\"/>
...
// str_replace("\\&quot;","&quot;",htmlentities($obra['titulo']))
  // Code for text documents:
	...
	echo "
<div class=\"download_button text\"><a href=\"../upload/".$row3['nuevo']."\" alt=\"".htmlentities($row['titulo'])." - ".$row['id'].$i."\"><img src=\"http://placehold.it/1280x300/000000/ffffff&text=cargar documento en Issuu\">".htmlentities($obra['titulo'])."</a></div>";

  // Code for video:
}else if(array_search($extension,$video) !== FALSE){
		echo "
<!-- <video src=\"../upload/".$row3['nuevo']."\" alt=\"".htmlentities($row['titulo'])." - ".$row['id'].$i."\" width=\"360\" height=\"202\">Get a real browser yo!</video> -->";

  // Code for audio:
}else if(array_search($extension,$audio) !== FALSE){
		echo "
<audio src=\"../upload/".$row3['nuevo']."\" alt=\"".htmlentities($row['titulo'])." - ".$row['id'].$i."\">Get a real browser yo!</audio>";

// All others:
}else{
	echo "
<div class=\"download_button file\"><a href=\"../upload/".$row3['nuevo']."\" alt=\"".htmlentities($row['titulo'])." - ".$row['id'].$i."\">".htmlentities($row['titulo'])."</a></div>";
}
      $i++;
      }     
      echo "
