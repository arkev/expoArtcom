...
<head>
  ...
  <style type="text/css" media="screen">
    ...
    .media{float: left;width: 360px;margin-left: 10px;margin-top: 3em;}
...
<?php
    $files = mysql_query("SELECT * 
      ...
      if(array_search($extension,$images) !== FALSE){
              echo '
            <img class="mediaimage" src="/i/w360'.$baseurl.'upload/'.$row3["nuevo"].'" alt="'.str_replace("\\&quot;","&quot;",htmlentities($obra['titulo'])).' - '.$obra["id"].$i.'"/>
      ...
      // str_replace("\\&quot;","&quot;",htmlentities($obra['titulo']))
        // Code for text documents:
      	...
      	echo '
      <div class="download_button text"><a href="'.$baseurl.'upload/'.$row3["nuevo"].'" alt="'.htmlentities($obra["titulo"]).' - '.$obra["id"].$i.'"><img src="http://placehold.it/360x300/000000/ffffff&text=Cargar documento en Issuu">'.htmlentities($obra["titulo"]).'</a></div>';

        // Code for video:
      }else if(array_search($extension,$video) !== FALSE){
      		echo '
      <video class="video-js vjs-default-skin" data-setup=\'{ "controls": true, "autoplay": false, "preload": "auto" }\' src="'.$baseurl.'upload/'.$row3["nuevo"].'" alt="'.htmlentities($obra["titulo"]).' - '.$obra["id"].$i.'" width="360" height="405">Get a real browser yo!</video>';
