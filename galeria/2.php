          ...
          while($row = mysql_fetch_assoc($obras)){
			...
			while($row3 = mysql_fetch_assoc($files2)){
				...
				if(array_search($extension,$images) !== FALSE){		 
			 	?>
			 		<img src="../slir/w280/expo-artcom/p12/upload/<?php echo $row3['nuevo'];?>" alt="<?php echo htmlentities($row['titulo'])." - ".$row['id'].$i?>" /> 
			 	<?php
			 		$encontroImagen = true;
			 		break;
			 	}	
			...
			if($encontroImagen == false){
				?>
				<img src="http://placehold.it/280x200&text=ver+<?php echo strtoupper($extension);?>" alt="<?php echo htmlentities($row['titulo'])." - ".$row['id'].$i?>" /> 
				<?php
			}
        ...
        style="border:none; overflow:hidden; width:150px; height:21px;" allowTransparency="true"></iframe>
								...
								<div id="imgs-container<?php echo $row['id'];?>" style="width:100%;background:black;">
									...
									?>
										...
										<img class='imgs' src='../slir/w280/expo-artcom/p12/upload/<?php echo $row4['nuevo'];?>'/>
										
									<?php
										   }else{
										   ?>
										   
										   <a target="_blank"href="../upload/<?php echo $row4['nuevo'];?>"><img class="imgs" src="http://placehold.it/280x200&text=ver <?php echo strtoupper($extension2);?>"></a>
										   <?php
										   }
										}
										$contador++;
...
<script>
  ...
  function getImagenes(){
		...
		var count = 1;
		while(count<=20){
			if(i>=boxes.length-1){//Si ya no hay imagenes que despleagar
				document.getElementById('append').style.display="none";
				break;
			}
			...
			if(proyectoAnterior!=str[10]){
				...
				if(imgEncontrada==true){
					...
					showImg = "../slir/w360/expo-artcom/p12/upload/"+str3[0];
				}
				...
				for(var n=0; n<boxes.length; n++){
					...
					if(arreglo[10]==str[10]){
						...
						if(extension.indexOf("png") != -1 || extension.indexOf("jpg") != -1 || extension.indexOf("jpeg") != -1 || extension.indexOf("gif") != -1){
							...
							document.getElementById("imgs-container"+str[10]).innerHTML = document.getElementById("imgs-container"+str[10]).innerHTML+" <img class='imgs' src='../slir/w360/expo-artcom/p12/upload/"+arreglo[0]+"'/>";
		...
		<script type="text/javascript">
			...
			function lightbox(id){
				...
				document.getElementById("fb-like"+id).src = "http://www.facebook.com/plugins/like.php?href=http://artcom.um.edu.mx/expo-artcom/p12/galeria/projects.php?id="+id+"&send=false&layout=button_count&width=150&show_faces=false&action=like&colorscheme=light&font=lucida+grande&height=21";
